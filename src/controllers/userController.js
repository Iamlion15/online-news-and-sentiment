...
class UserController {
    ...
    static async modifyReview(req, res) {
        const review = await reviewModel.findOneAndUpdate({ _id:req.body.id });
        try {
    ...
    static async getReviews(req, res) {
        const personel=await userModel.findOne({nID:req.user});
        try {
            const reviews = await reviewModel.find({ user: personel._id }).populate('news').populate('user');
    ...
    static async getNewsReview(req, res) {
        const personel=await userModel.findOne({nID:req.user});
        try {
            const reviews = await reviewModel.find({ user: personel._id, news: req.body.news }).populate('news').populate('users');
            res.status(200).json(reviews)
        } catch (error) {
            res.status(404).json(error.message)
        }
